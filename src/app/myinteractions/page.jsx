...
const MyInteractions = () => {
    ...
    const fetchMyComments = async () => {
      ...
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_SERVER_URL}/user-comments?email=${session.user.email}`,
        );
        if (res.ok) {
  ...
  return (
    <div className="max-w-[1200px] w-full mx-auto px-6 pt-30 pb-15">
      <h1 className="text-[25px] font-black mb-10 text-slate-900 dark:text-white">
        My <span className="text-[#C6D62E]">Interactions</span>{" "}
      </h1>
                  ...
                  <div className="space-y-1 w-full">
                    ...
                    {comment.ideaTitle && (
                      <span className="text-sm text-slate-500 dark:text-slate-400">
                        Commented on:{" "}
                        <span className="text-[#C6D62E] font-semibold">
                          {comment.ideaTitle}
                        </span>
                      </span>
                    )}
